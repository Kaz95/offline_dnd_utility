...
class TestDatabaseUnitIntegration(unittest.TestCase):
    ...
    def test_add_account_row(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.add_account_row(), ('kaza', 'tree'))
    ...
    def test_add_character_row(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.add_character_row(), (1, 'char name', 5000))
    ...
    def test_add_inventory_row(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.add_inventory_row(), (1, 1, 'inv name'))
    ...
    def test_add_item_row(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.add_item_row(), (1, 1, 1, 'item name',
                                                                                           'api url', 1, 1))
    ...
    def test_add_store_item(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.add_store_item(),
                                                                  ('club', 'some api', 1, 'bs'))
    ...
    def test_delete_item(self):
        with mock.patch('database.sqlite3') as mocksql:
            ...
            mocksql.connect().cursor().execute.assert_called_with(sql.delete_item(), ('Club', 1))

    def test_delete_all_character_items(self):
        with mock.patch('database.sqlite3') as mocksql:
            conn = mocksql.connect()
            database.delete_all_character_items(conn, 1)
            mocksql.connect().cursor().execute.assert_called_with(sql.delete_all('items', 'character_id'), (1,))

    def test_delete_character_inventories(self):
        with mock.patch('database.sqlite3') as mocksql:
            conn = mocksql.connect()
            database.delete_character_inventories(conn, 1)
            mocksql.connect().cursor().execute.assert_called_with(sql.delete_all('inventories', 'character_id'),
                                                                  (1,))
